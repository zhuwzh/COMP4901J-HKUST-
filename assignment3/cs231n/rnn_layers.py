...
def rnn_backward(dh, cache):
    ...
    - dh: Upstream gradients of all hidden states, of shape (N, T, H)
          Notice this dh[:,t:,] = dLoss_t / dh[:,t,:] instead of dLoss / dh[:,t,:]
...
def word_embedding_backward(dout, cache):
    ...
    dout_re = np.reshape(dout, (N*T,D))
    x_re = np.reshape(x,(N*T))
    
    for v in range(V): 
        dW[v] = np.sum(dout_re[x_re == v], axis=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return dW
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    stack = prev_h.dot(Wh) + x.dot(Wx) + b # (N,4H)
    i = sigmoid(stack[:,:H])
    ...
    cache = i,f,o,g,prev_c,next_c,prev_h,x,Wx,Wh
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    i,f,o,g,prev_c,next_c,prev_h,x,Wx,Wh  = cache
    
    dh_dc = o * (1-np.tanh(next_c)**2)
    dnext_c += dnext_h * dh_dc
    
    dprev_c = f * dnext_c
    
    dc_df = prev_c * dnext_c
    dc_di = g * dnext_c
    dc_dg = i * dnext_c
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dnext_h += dh[:,t,:]
        dx[:,t,:], dnext_h, dnext_c, dWxt, dWht, dbt = lstm_step_backward(dnext_h, dnext_c, cache[t])
        dWx += dWxt
        dWh += dWht
        db += dbt
