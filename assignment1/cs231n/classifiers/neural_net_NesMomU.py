...
class TwoLayerNet_NesMomU(object):
  '''
  The difference between NesMom and NesMomU(Update) is we calculate 
  the gradient and loss at the same point, which is W_ahead.
  '''
  ...
  def LsGd(self, X, y=None, reg=0.0):
    """
    LsGd function returns the a tuple with:
        loss at current estmate of W (intrinsically W_ahead)
        gradient at W_ahead
    """
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      loss,grads = self.LsGd(X_batch, y=y_batch, reg=reg)
      ...
      self.params['W1'] += -mu * v_W1_t + (1+mu) * v_W1
      self.params['b1'] += -mu * v_b1_t + (1+mu) * v_b1
      self.params['W2'] += -mu * v_W2_t + (1+mu) * v_W2
      self.params['b2'] += -mu * v_b2_t + (1+mu) * v_b2
